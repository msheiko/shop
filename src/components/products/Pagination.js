    ...
    return (
        <nav>
            <ul className="pagination">
                ...
                {_.range(1,meta.total_pages).map( (p)=> <li className="page-item"><NavLink  className={() => (p===meta.current_page ? 'active page-link' : 'page-link')} to={`/products?page=${p}`}>{p}</NavLink></li>)}
                <li className="page-item">
